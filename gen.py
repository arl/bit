...
def write_unsigned(n: int) -> str:
    ...
    a(f"type {nm} {back}")
    a(f"")
    a(f"// As{nm} returns a {nm} representing v reduced to {n} bits.")
    a(f"func As{nm}[T Unsigned](v T) {nm} {{ return {nm}.cast({nm}(v)) }}")
    a(f"")
...
def write_signed(n: int) -> str:
    ...
    a(f"")
    a(f"// As{nm} returns a {nm} representing v reduced to {n} bits.")
    a(f"func As{nm}[T Signed](v T) {nm} {{ return {nm}.cast({nm}(v)) }}")
    a(f"")
    a(f"func ({nm}) nbits() int        {{ return {n} }}")
